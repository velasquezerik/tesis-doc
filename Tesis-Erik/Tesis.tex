...
En la figura \ref{ejecucionGalatea} podemos observar la ejecución del modelo "SimpleTeller", que se encuentra entre los ejemplos básicos de GALATEA. En este modelo define la taquilla de un banco, que posee una entrada ``Gate (I)" y una salida "Exit (E)". Las personas que entran llegan a un cajero de banco a intervalos de tiempo tomadas al azar de una distribución exponencial con la media ``InArrTime". Se sirven uno a la vez. Los tiempos de servicio se toman de una distribución gaussiana con media ``MeSerTime" y desviación estándar ``DeSerTime".
...
Todas estas llamadas están escritas en Python y usando la definición de ORM, este manejador encapsula las llamadas a la base de datos y genera automáticamente los SQLs específicos de cada llamada.
...
\begin{itemize}
	\item \textbf{Pantalla del Dashboard (Inicio)}: Esta es la pantalla de inicio de un usuario registrado, en ella encontramos la información principal y las distintas opciones de navegación. Como eje central se encuentran los archivos y carpetas de nuestro sistema raíz.
	\begin{figure}[H]
		\centering
	...
	\item \textbf{Pantalla Observación de Archivo}: Observamos la información contenida en el archivo y  poseemos un editor web para editarlo, al igual que todas las opciones que podemos manejar sobre el mismo.
	\begin{figure}[H]
		\centering
	...
	\item \textbf{Pantalla Integración por Sockets}: Esta pantalla solo es accedida por usuarios administradores del sistema, en ella tenemos la visualización de la integración realizada con GALATEA mediante sockets. Tenemos una interfaz que cuenta con la implementación del protocolo de comunicación definido para esta integración, podemos revisar toda la información en el capítulo \ref{integracionGALATEA}.
	\begin{figure}[H]
		\centering
...
Su función es procesar la simulación y disparar los eventos que serán capturados por el manejador de eventos.
...
Es el encargado de controlar todos los aspectos principales del programa servidor. Primero inicia el programa en un puerto específico, e inicializa el hilo de simulación y el manejador de eventos.
...
Comenzando el tSim=10 y terminando con un tSim=100, con un incremento de 10. Se realizaron 10 pruebas, obteniendo en todas la misma información entre simulaciones.
...
Comenzando el InArrTime=4.0 y terminando con un InArrTime=8.0, con un incremento de 1.0. Se realizaron 4 pruebas, obteniendo en todas la misma información entre simulaciones.
...
Comenzando el MeSerTime=3.5 y terminando con un MeSerTimerTime=7.5, con un incremento de 1.0. Se realizaron 4 pruebas, obteniendo en todas la misma información entre simulaciones.
...
El cliente Python se desarrolló, para ejecutar cuatro instrucciones básicas del protocolo de comunicación: ``Start" comenzar la simulación, ``Set" modificar el valor de una variable dentro de la simulación, ``Get" obtener el valor de una variable dentro de la simulación y ``Exit" terminar la simulación.
...
El cliente C se desarrolló, para ejecutar cuatro instrucciones básicas del protocolo de comunicación al igual que el cliente Python: ``Start" comenzar la simulación, ``Set" modificar el valor de una variable dentro de la simulación, ``Get" obtener el valor de una variable dentro de la simulación y ``Exit" terminar la simulación.
...
La creación de archivos es un objetivo fundamental dentro del proyecto, por lo que es necesario garantizar su correcto funcionamiento. La prueba consistió en la creación de archivos usando el sistema web y verificar su correcta creación en el servidor.
...
Observamos que la carpeta \textbf{FolderTest} fue creada con éxito. Y se encuentra dentro del directorio principal del admin.
...
El sistema web de simulación, cuenta con una gama de funcionalidades que explicaremos a continuación:
...
El sistema permite el control de dos tipos de usuarios (Administradores y usuarios normales).
...
Solo los administradores del sistema pueden acceder a esta página y visualizar los usuarios. Contamos con una serie de opciones en esta página. El botón "\textbf{New Admin}" y "\textbf{New User}", te envían a la pagina respectiva para crear ese tipo de usuario. Cada uno de los usuarios listados, cuentan con la opción de "\textbf{Delete}" y "\textbf{Edit}", que desactiva al usuario del sistema y permite editar sus datos respectivamente.
...
Agregar un usuario con los privilegios de usuario normal del sistema. Ésta página la accedemos desde el botón "\textbf{New User}" en la página \ref{paginaListaUsuarios}.
...
Agregar un usuario con los privilegios de administrador del sistema.  Ésta página la accedemos desde el botón "\textbf{New Admin}" en la página \ref{paginaListaUsuarios}.
...
Editamos la información principal del usuario del sistema, al igual la cantidad de espacio en disco asociado. Ésta página la accedemos desde el botón "\textbf{Editar}" en la página \ref{paginaListaUsuarios}, asociado a cada usuario.
...
Observamos los logs de archivos. Los cambios principales de archivos que los usuarios han realizado en el sistema. Ésta página la accedemos desde el botón "\textbf{Logs Files}" en la página \ref{paginaInicioAdmin}. Solo Usuarios con el rol de \textbf{Administradores} pueden acceder a ella.
...
Observamos los logs de carpetas. Los cambios principales de carpetas que los usuarios han realizado en el sistema. Ésta página la accedemos desde el botón "\textbf{Logs Folders}" en la página \ref{paginaInicioAdmin}. Solo Usuarios con el rol de \textbf{Administradores} pueden acceder a ella.
...
Observamos los logs de relaciones. Los cambios principales de las relaciones que los usuarios han realizado en el sistema. Ésta página la accedemos desde el botón "\textbf{Logs Relations}" en la página \ref{paginaInicioAdmin}. Solo Usuarios con el rol de \textbf{Administradores} pueden acceder a ella.
...
En el perfil de usuario nos encontramos con la información básica del usuario. Podemos modificar los \textbf{datos básicos} o \textbf{cambiar la contraseña}.
...
Listado de nuestros amigos en el sistema. Ésta página la accedemos desde el botón "\textbf{Friends}" en la página \ref{paginaInicioUser}.
...
Enviamos notificaciones de amistad a usuarios. Ésta página la accedemos desde el botón "\textbf{ Get Friends}" en la página \ref{paginaInicioUser}.
...
Verificamos las notificaciones de amistad que nos han enviado. Ésta página la accedemos desde el botón "\textbf{Friends Notifications}" en la página \ref{paginaInicioUser}.
...
Verificamos las notificaciones de archivos compartidos que nos han enviado. Ésta página la accedemos desde el botón "\textbf{Share Files Notifications}" en la página \ref{paginaInicioUser}.
...
Accedemos a nuestros archivos compartidos. Ésta página la accedemos desde el botón "\textbf{Share Files}" en la página \ref{paginaInicioUser}.
...
Accedemos a la información que contienen los archivos dentro del sistema. Nos permite \textbf{editarla} y \textbf{guardarla}. Accedemos dándole click al botón a un archivo en la página \ref{paginaInicioUser}.
...
Modificamos la permisología de acceso del archivo. Accedemos dándole click al botón "\textbf{File Permission}" en la página \ref{paginaUserShowFile}. Nos permite manejar cuatro permisologías (\textbf{Private, show, edit y Public}).
...
Observamos las opciones de compilación que posee el archivo. Accedemos a estas opciones en la página \ref{paginaUserShowFile}. 
...
Ejecutamos el modelo de simulación dándole click a la opción "\textbf{Execute File}" y pasando los argumentos que se deseen al modelo en el input "\textbf{Arguments}". Accedemos a estas opciones en la página \ref{paginaUserShowFile}. 
