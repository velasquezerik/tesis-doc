...
\resumen{
Se implementó y desplegó como servicio, una aplicación utilizada para modelar y simular eventos discretos en sistemas distribuidos, utilizando como medio los servicios web. El objetivo principal de este proyecto ha sido, mediante el uso de un sistema web, solucionar un problema existente entre los usuarios y usuarias del Centro de Simulación y Modelado (CESIMO) de la Universidad de los Andes, que suelen tener muchas dificultades para ejecutar los modelos de simulación en la plataforma de simulación GALATEA, especialmente al momento de configurar y activar todas sus funcionalidades. Mediante el patrón de diseño de software modelo vista controlador (MVC) y usando el framework de desarrollo web Django; se desarrolló el sistema web, que usando como motor principal a GALATEA, permite crear, controlar y simular proyectos de simulación. Al igual se creó un sistema de gestión y administración de archivos, la cual permite compartir y editar archivos entre los distintos usuarios y usuarias que hacen uso del sistema web de simulación.
}
...
% con o sin comentar
% ***************************************************************** %
\dedicatoria{Le dedico este trabajo a mi abuela Georgina Spirutova, que aunque ya no me acompaña en este mundo, su recuerdo, su carácter y sus enseñanzas siempre me acompañaran en todos los caminos que recorra en mi vida.\\
}
...
Primero que nada le agradezco a mis padres por ser mi ejemplo y modelo a seguir. Por enseñarme que con trabajo duro y dedicación se puede lograr cualquier cosa. Por siempre estar allí para mi, ayudarme en todo lo que yo necesitaba y velar por que nunca me faltara nada. A mis hermanas Adriana y Arianna que siempre han estado acompañándome en cada paso importante de mi vida. \\ \\
A mi tío Ivan y mi tía Yudith por abrirme las puertas de su casa y permitirme formar parte de su familia durante todos mis estudios universitarios. Al igual enseñarme muchos valores que siempre tendré conmigo. A mis hermanos Andy y Alan, que no serán mis hermanos de sangre, pero son mis hermanos de vida.\\ \\
Le agradezco al resto de mi familia, mi tía Maria Isabel, mi tía Mariela, mi abuelo Manuel, mi abuelo Rosauro, mi abuela Dora, por mencionar unos pocos.\\ \\
A mis compañeros de clase, mis amigos, como lo son: Mariangely, Juan Carlos, Juan Pablo, Encel, Jorge, Vanessa, Rosa, Manuel, que siempre estaban en las buenas y las malas. Al igual a Heyzel por acompañarme y brindarme su apoyo incondicional durante el desarrollo de éste trabajo.\\ \\
A mi tutor Jacinto y Co-tutor Kay que me permitieron trabajar con ellos, apoyarme y ayudarme en todo lo que necesite para culminar el proyecto.\\ \\
...
En su tesis de pre-grado \citep{tesisGustavo}, buscó implementar y desplegar como servicio, una aplicación utilizada para modelar y simular eventos discretos en sistemas distribuidos, utilizando como medio los servicios web. Mediante el uso del paradigma de arquitectura orientada a servicios (SOA: Service Oriented Architecture. Pero ese trabajo no alcanzó a resolver la complejidad de un servicio basado en SOA y tampoco de lograr la integración con GALATEA.
...
La preocupación por los sistemas distribuidos y de cómo diferentes máquinas podían comunicarse entre sí surgió en la década de los 80. Hasta ese momento, era suficiente con que las aplicaciones de un mismo ordenador pudieran establecer una comunicación. 
...
En cuanto al otro aspecto, consiste en verificar que la integridad de la información y la identidad del usuario estén garantizados, en especial cuando el usuario desee enviar algún dato, o realizar alguna consulta, por lo cual el sistema debe validar a dicho usuario previamente.
...
El segundo ciclo se enfocó en el desarrollo de un administrador de archivos y carpetas dentro de nuestro sistema web, permitirle a los usuarios crear, modificar y eliminar archivos y carpetas.
...
\item Capítulo II. Marco teórico-funcional.
...
En esta sección se exponen las distintas pruebas realizadas a la aplicación web, definición, implementación y el resultado, así mismo hacemos un estudio de los resultados obtenidos.
...
En la figura \ref{ejecucionGalatea} podemos observar la ejecución del modelo "SimpleTeller", que se encuentra entre los ejemplos básicos de GALATEA. En este modelo define la taquilla de un banco, que posee una entrada ``Gate (I)" y una salida "Exit (E)", como se observa en la figura \ref{codigoTaquillaGalatea}. Las personas que entran llegan a un cajero de banco a intervalos de tiempo tomadas al azar de una distribución exponencial con la media ``InArrTime". Se sirven uno a la vez. Los tiempos de servicio se toman de una distribución gaussiana con media ``MeSerTime" y desviación estándar ``DeSerTime".

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.5]{img/codigoTaquillaGalatea.png}
	\caption{Código GALATEA del modelo SimpleTeller.}
...
\begin{figure}[H]
	...
	\caption{Ejecución del modelo SimpleTeller.}
...
Los modelos dentro de nuestro sistema son representados en clases, las cuales a su vez representan los objetos a ser almacenados dentro de nuestra base de datos. Los modelos de datos ``Folder" y ``File" mostrados en el segmento de código anterior, se representan junto con sus atributos, tipos de datos y relaciones.
...
Django ha demostrado ser escalable y flexible, se sabe de instancias de Django atendiendo ráfagas de cincuenta mil peticiones por segundo \citep{tesisJesus}, además cuenta con una extensa comunidad de colaboradores y amplia documentación.
...
El Mapeo Objeto-Relacional (ORM por sus siglas en ingles) es un método para interacturar con bases de datos relacionales desde el paradigma de la programación orientada a objetos, de esta manera es posible aprovechar conceptos como herencia y polimorfismo. Es una técnica de programación para convertir datos entre el sistema de tipos utilizado en un lenguaje de programación orientado a objetos y la utilización de una base de datos relacional como motor de persistencia. En la práctica esto crea una base de datos orientada a objetos virtual, sobre la base de datos relacional. Esto posibilita el uso de las características propias de la orientación a objetos (básicamente herencia y polimorfismo). A continuación observamos un segmento de código que hace uso del ORM de Djando usado en nuestro sistema:

\begin{lstlisting}
users = User.objects.all()    #obtener todos los usuarios registrados en la DB
user = User.objects.get(id=user_id)	#obtener la información de un usuario específico
folders_count = Folder.objects.filter(user_id = user_id).count() #contar las carpetas que pertenecen al usuario
folders = Folder.objects.filter(user_id = user_id) #obtener todas las carpetas asociadas al usuario
files = File.objects.filter(folder__user_id = user_id) #obtener todos los archivos asociados al usuario

\end{lstlisting}

Todas estas llamadas están escritas en Python y usando la definición de ORM, este manejador encapsula las llamadas a la base de datos y genera automáticamente los SQLs específicos de cada llamada. Los SQLs relacionados con las llamadas mostradas anteriormente, se pueden observar en el siguiente segmento de código:

\begin{lstlisting}
select * from users     #users = User.objects.all()
select * from users where id = "user_id" #user = User.objects.get(id=user_id)
select count(*) from folders where user_id = "user_id" #folders_count = Folder.objects.filter(user_id = user_id).count()
select * from folders where user_id = "user_id" #folders = Folder.objects.filter(user_id = user_id)
select * from files where  #files = File.objects.filter(folder__user_id = user_id)
...
Para crear la comunicación asíncrona con el servidor, nuestras llamadas de compilación y ejecución de modelos de simulación se realiza mediante una llamada AJAX. Esto nos permite continuar realizando tareas sobre nuestros modelos de simulación, mientras el servidor ejecuta esos procesos de compilación y ejecución por su lado.

Estos son los elementos conceptuales y las correspondientes realizaciones funcionales que hemos empleado para desarrollar este proyecto. En el capítulo \ref{desarrolloSistemaWeb} se explica cómo se ha realizado ese trabajo. 

\chapter{Desarrollo del Sistema WEB}\label{desarrolloSistemaWeb}
...
\begin{figure}[H]
	...
	\caption{Componentes del Sistema de Simulación Web GALATEA.}
...
La arquitectura del sistema web de simulación GALATEA consiste en cuatro capas como se observa en la figura \ref{arquitecturaAplicacion}, la capa superior o capa de presentación se ejecuta en el navegador del cliente, para ver los resultados obtenidos de la simulación, visualización de archivos, entre otras acciones que pude realizar el usuario; esta capa se comunica con la capa dos o capa de servicio, que es ejecutada por el servidor web y es la responsable de atender las peticiones de los usuarios del sistema; la capa tres es la encargada de las peticiones a los otros módulos que conforman el sistema, comunicación con el módulo de manejo de archivos e integración con el simulador GALATEA; la capa cuatro o capa de datos aloja los datos de la aplicación como usuarios, archivos, carpetas, modelos, trazas y datos de simulaciones, entre otros.

\subsection{Capa 1: Presentación}

La capa de presentación es la interfaz gráfica que permite la interacción del usuario con el sistema de simulación Web. Esta capa consiste en los despliegues de navegadores en cualquier dispositivo conectado a Internet y su propósito es ofrecer la usuario una interfaz gráfica para ingresar al sistema, configurar y manejar simulaciones, observar resultados, entre otros.
...
Las relaciones de usuarios se maneja mediante el modelo \textit{Relationship}, lo cual nos permite definir la relación de \textit{amistad} entre dos usuarios. Esto nos permitirá compartir archivos entre usuarios.

Los archivos son manejados usando el modelo \textit{File}, que permite definir la información básica y permisología de acceso a los mismos.

Las carpetas están definidas mediante el modelo  \textit{Folder}, que contiene la información básica y jerárquica de carpetas. Otro modelo muy relacionado es \textit{DiskSpace}, que nos permite llevar el control de espacio en disco usado por el usuario.
...
Las siguientes tablas resumen las entidades y la función de las mismas dentro del sistema web: La tabla \ref{tabla-manejo-usuarios} define las entidades relacionadas al manejo de usuarios, la tabla \ref{tabla-manejo-archivos-carpetas} define las entidades relacionadas al manejo de archivos y carpetas, por último la tabla \ref{tabla-manejo-archivos-carpetas-compartidos} define las entidades relacionadas al manejo de archivos y carpetas compartidos.
...
\begin{itemize}
	...
	\item \textbf{User - Folder (1-n)}: Un usuario puede poseer una o mas carpetas, pero una carpeta pertenece a un solo usuario.
	\item \textbf{User - DiskSpace (1-1)}: Un usuario tiene un solo espacio en disco asociado y configurado.
	\item \textbf{Folder - File (1-n)}: Una carpeta puede tener muchos archivos almacenados, pero un archivo tiene una carpeta asociada.
	\item \textbf{User - ShareFile (1-n)}: Un usuario puede poseer uno o mas archivos compartidas, pero un archivo pertenece a un solo usuario.
	\item \textbf{File - ShareFile (1-n)}: Un archivo puede estar compartido con muchos usuarios.
	\item \textbf{User - ShareFolder (1-n)}: Un usuario puede poseer una o mas carpetas compartidas, pero una carpeta pertenece a un solo usuario.
	\item \textbf{Folder - ShareFolder (1-n)}: Un carpeta puede estar compartida con muchos usuarios.
	\item \textbf{User - Relationship (1-n)}: Un usuario puede poseer una o mas relaciones de amistad con otros usuarios, pero la relación se define con un solo usuario.
\end{itemize}
...
Todas las plantillas heredan de una plantilla base, ésta incluye la declaración del archivo HTML, la cabecera y define bloques que pueden ser sobrescritos por las plantillas ``hijas". Los bloques principales definidos en nuestro diseño son: \textbf{(1) Bloque de título} permite definir el título de la página, \textbf{(2) Bloque de navegación} permite un acceso rápido a los sitios accedidos previamente, \textbf{(3) Bloque de contenido} bloque principal con el contenido de la página, \textbf{(4) Bloque de JavaScript} para incluir archivos y métodos JavaScript. La plantilla general hereda de la \textit{plantilla base} y esta a su vez incluye la información de la \textit{barra lateral} y el \textit{header} de la página.

A continuación se muestran las principales pantallas de la interfaz de usuario del servicio web:

\begin{itemize}
	\item \textbf{Pantalla del Dashboard (Inicio)}: Esta es la pantalla de inicio de un usuario registrado, en ella encontramos la información principal y las distintas opciones de navegación. Como eje central se encuentran los archivos y carpetas de nuestro sistema raíz, la cual se observa en la figura \ref{pantalla1}.
	\begin{figure}[H]
		\centering
	...
	\item \textbf{Pantalla Observación de Carpeta}: En ella encontramos todos los archivos y carpetas que se encuentran dentro de nuestra carpeta, al igual observamos todas las opciones que podemos ejecutar dentro de ella, esto se observa en la figura \ref{pantalla2}.
	\begin{figure}[H]
		\centering
	...
	\item \textbf{Pantalla Observación de Archivo}: Observamos la información contenida en el archivo y  poseemos un editor web para editarlo, al igual que todas las opciones que podemos manejar sobre el mismo, como se observa en la figura \ref{pantalla3}.
	\begin{figure}[H]
		\centering
	...
	\item \textbf{Pantalla Compilación y Ejecución}: Aquí podemos acceder a las opciones de compilación y ejecución de nuestro archivo, junto con los resultados obtenidos de esos procesos, la cual se observa en la figura \ref{pantalla4}.
	\begin{figure}[H]
		\centering
	...
	\item \textbf{Pantalla Control de Usuarios}: Esta pantalla solo es accedida por usuarios administradores del sistema, en ella tenemos la visualización de usuarios y podemos acceder a las distintas opciones de creación y edición de usuarios , observado en la figura \ref{pantalla5}.
	\begin{figure}[H]
		\centering
	...
	\item \textbf{Pantalla Integración por Sockets}: Esta pantalla solo es accedida por usuarios administradores del sistema, en ella tenemos la visualización de la integración realizada con GALATEA mediante sockets. Tenemos una interfaz que cuenta con la implementación del protocolo de comunicación definido para esta integración, podemos revisar toda la información en el capítulo \ref{integracionGALATEA}, dicha pantalla se observa en la figura \ref{pantalla6}.
	\begin{figure}[H]
		\centering
		\includegraphics[scale=0.2]{img/socketsIntegration.png}
		\caption{Pantalla Integración por Sockets.}
		\label{pantalla6}
	\end{figure}
\end{itemize}
...
Ya que la aplicación web consiste en un conjunto grande de vistas, debemos diseñar URLs que apunten a cada una de ellas de forma ordenada y lógica. Para esto, se ha usado una convención en la que la unidad principal de operación como archivos, carpetas, usuarios anteceden a la operación real que se está llamando; a continuación se muestran ejemplos de las reglas usadas para diseñar las URLs:
...
El framework de desarrollo web Django usado para desarrollar este proyecto nos permite tener una interacción directa con los archivos y carpetas. Esto debido a que el lenguaje principal en que se encuentra escrito (Python), el cual es un lenguaje de programación potente y versátil para desarrollar cualquier tipo de tareas. Por ende, nuestro administrador de archivos y carpetas esta escrito en python, haciendo uso de las librerías de control archivos y carpetas que el lenguaje nos proporciona.
...
\begin{table}[H]
	...
	{\fontsize{10pt}{\baselineskip}\selectfont
		\begin{tabular}[t]{|l|c|}
			...
			\textit{Show (1) } & Otros usuarios pueden acceder a él solo para lectura \\\hline
			\textit{Edit (2) } & Otros usuarios pueden acceder a él para lectura y escritura  \\\hline
			\textit{Public (3) } & Otros usuarios pueden acceder a él para lectura/escritura y eliminación  \\\hline
		\end{tabular}
	}
...
\begin{itemize}
	...
	\item Edit File: permite editar la información que se encuentra en el archivo.
	\item Move File: permite mover el archivo desde su carpeta origen a una nueva carpeta destino.
	\item Delete File: permite eliminar el archivo de nuestro sistema.
\end{itemize}
...
\begin{itemize}
	...
	\item Edit Folder: permite editar la información básica de la carpeta.
	...
	\item Delete Folder: permite eliminar toda la información de la carpeta junto con sus archivos y carpetas internos.
\end{itemize}

Debido a que el espacio en disco físico es limitado, cada usuario tiene una porción de espacio asociado para su manejo de archivos y carpetas. Este espacio es constantemente monitoreado por el administrador de archivos y es una de las restricciones principales para el correcto funcionamiento del servicio. Esta porción de espacio en disco es fácilmente administrable por el usuario administrador de nuestro sistema web.

El sistema manejador de archivos y carpetas se desarrolló usando llamadas a los métodos de manejo de archivos y carpetas que proporciona el sistema operativo donde el sistema se encuentre instalado. Esto permite que dependiendo el sistema operativo usado y los recursos de hardware disponibles podemos hacer uso de las nuevas tecnologías disponibles como: "hot swap", modificar tamaños de las particiones en tiempo real y otras tecnologías que los nuevos servidores nos permiten para el manejo de archivos y carpetas.
...
\chapter{Integración con GALATEA} \label{integracionGALATEA}

Debido a que GALATEA esta desarrollado en Java y nuestro sistema web esta escrito en Python, necesitamos un mecanismo de comunicación para ambos lenguajes. Para esto se desarrolló dos mecanismos de comunicación como lo son: (1) Subprocesos y (2) Sockets. El primero de ellos lo implementamos realizando invocación a un subproceso
o proceso externo, enviándole los parámetros necesarios para su ejecución, este enfoque es el actualmente implementado en nuestro sistema web y esta bien especificado en la sección \ref{llamadaProcesos}. La segunda implementación es una comunicación entre procesos vía sockets, el cual atiende las peticiones en un puerto de comunicación específico, lo cual nos permite comunicar nuestro sistema Python con el motor de simulación escrito en Java, como se especifica en la sección \ref{sockets}. 

\section{Subprocesos} \label{llamadaProcesos}

El módulo de subproceso en Python  permite ejecutar nuevos procesos, tomando en cuenta las operaciones de entrada/salida/error y así obtener la información de retorno. Lo utilizan para ejecutar un comando shell o un archivo ejecutable en Python, \citep{subproccess}. En la figura \ref{subprocesoWork} observamos la estructura y ejecución de un subproceso usado en nuestro sistema.
...
\begin{itemize}
	...
	\item Ejecutar la simulación: Finalmente ejecutamos la simulación con sus diferentes parámetros de entrada. Es decir, invocamos el modelo de simulación con ciertos parámetros.
\end{itemize}

Cada uno de estos pasos es ejecutado mediante una llamada a un subproceso dentro de nuestro sistema. El resultado de cada una de los pasos es obtenido como valor de retorno de la misma y es mostrado al usuario. A continuación se encuentra un fragmento de código escrito en Python en el que se define la ejecución de un subproceso dentro del sistema:
...
En el fragmento de código mostrado anteriormente se ejecuta un modelo de simulación dado y el resultado es retornado para ser mostrado al cliente.
...
En la figura \ref{socketWork} observamos un flujo típico de eventos cuando se define una conexión por sockets: 1) Creamos un punto de conexión entre el cliente y el servidor mediante sockets. 2)La API listen() indica una disposición a aceptar peticiones de conexión de cliente. Cuando se emite una API listen() para un socket, ese socket no puede iniciar activamente solicitudes de conexión. La API listen() se emite después de asignar un socket con una API socket() y la API bind() vincula un nombre al socket. Se debe emitir una API listen() antes de que se emita una API accept(). 3) La aplicación cliente utiliza una API connect() en un socket de flujo para establecer una conexión con el servidor. 4) La aplicación de servidor utiliza la API accept() para aceptar una solicitud de conexión de cliente. El servidor debe emitir las APIs bind() y listen() con éxito antes de poder emitir una API accept(). 5) Cuando se establece una conexión entre sockets de flujo (entre cliente y servidor), puede utilizar cualquiera de las API de transferencia de datos API de socket. Los clientes y los servidores tienen muchas API de transferencia de datos de las que elegir, como send(), recv(), read(), write() y otros. 6) Cuando un servidor o cliente desea detener las operaciones, emite una API close() para liberar los recursos del sistema adquiridos por el socket.
...
El programa cliente lo podemos estructurar en tres bloques principales:

\begin{itemize}
	\item Iniciar Conexión: Lo principal es crear una comunicación directa con el programa servidor, por lo que, verificamos la conexión y nos conectamos al socket específico. Garantizar la conexión es importante para poder mantener un control continuo de la simulación.
	\item Envío de instrucciones: utilizando el protocolo de comunicación definido en la sección \ref{protocoloComunicacion}, enviamos al programa servidor una serie de instrucciones de prueba para verificar el correcto funcionamiento de la simulación.
	\item Finalizar conexión: luego de ejecutar toda la simulación y obtener los valores deseados, finalizamos la conexión con el programa servidor.
\end{itemize}
...
Este programa esta definido por tres hilos de ejecución como observamos en la figura \ref{estructuraProgramaServidor}:

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.4]{img/estructuraProgramaServidor.png}
...
Debido a, que en los nuevos desarrollos de GALATEA para cuando este trabajo se realizó, se esta desarrollando el manejo de eventos dentro de GALATEA, los cuales son disparados cuando situaciones particulares dentro de la simulación ocurren. En nuestra integración se tomó en cuenta y se implementó ese manejo de eventos.

La función de el manejador de eventos es capturar todos los eventos que son creados dentro de la simulación, procesarlos y enviarlos nuevamente al programa cliente, para su posterior observación. Los eventos y su procesamiento ocurren en tiempo real, es decir, a penas el evento es creado se captura y se procesa. A continuación encontramos un fragmento de código en el que se define el manejo de eventos:
...
Este manejador de eventos esta definido por una clase de JAVA, en la cual encontramos el método ``modelReceived" el cual es el encargado de manejar el evento dentro del proceso de simulación.

\subsubsection{Hilo de Simulación}

Éste módulo es el modelo de simulación implementado como un Thread de Java \citep{javaThreads}. Este hilo encapsula toda la información del modelo de simulación (variables y métodos), al igual todos los métodos de comunicación con GALATEA.

Su función es procesar la simulación y disparar los eventos que serán capturados por el manejador de eventos. A continuación encontramos el código para el manejo del hilo de simulación, definido dentro de nuestra integración:
...
En el código observamos que hacemos uso de las clases principales de JAVA para el manejo de hilos de ejecución, al igual de los métodos necesarios para su control y ejecución.

\subsubsection{Cuerpo Principal}

Es el encargado de controlar todos los aspectos principales del programa servidor. Primero inicia el programa en un puerto específico, e inicializa el hilo de simulación y el manejador de eventos. En el siguiente fragmento de código observamos todas las definiciones de las variables necesarias y la inicialización de las mismas para su funcionamiento:
...
Se encarga de recibir todas las solicitudes del programa cliente, mediante el protocolo de comunicación definido en la sección \ref{protocoloComunicacion}, y ejecutar esas peticiones sobre el thread de simulación, al igual controlar todo el manejo de excepciones que puedan ocurrir.

\subsection{Protocolo de Comunicación} \label{protocoloComunicacion}
...
\begin{itemize}
	...
	\item stop: detiene definitivamente la simulación (si el simulador esta programado para detección temprana de la orden esto puede ocurrir en cualquier momento).
...
Esta integración de GALATEA mediante sockets se desarrolló como una propuesta experimental, con el objetivo de permitir una interacción en tiempo real con el proceso de simulación y el usuario simulista. Teniendo en mente este principio se definió el protocolo de comunicación explicado anteriormente, el cual englobamos las operaciones básicas de control sobre la simulación y el acceso a las variables que se manejan en la misma. Para desarrollar esta integración se utilizó como base el modelo de simulación \textbf{SimpleTeller}. El cual fue adaptado para que cumpliera con las funcionalidades explicadas anteriormente.

Las pruebas de ejecución de todas estas formas de integración de el portal Web con GALATEA se describirán a continuación en el capítulo \ref{pruebas}.

\chapter{Pruebas} \label{pruebas}

\section{Pruebas de Integración con GALATEA caso integración asíncrona}

Para el manejo de las pruebas de integración se definió un modelo de simulación de prueba (\textbf{SimpleTeller}). Nuestro objetivo principal es que los modelos de simulación muestren la misma información de la simulación que muestran cuando son ejecutados en  GALATEA en la consola de una maquina aislada. Para ello todas las pruebas sobre el modelo escogido fueron ejecutadas en el sistema web y en GALATEA directamente.

\subsection{Variación de los parámetros de Simulación}

El modelo de simulación escogido (\textbf{SimpleTeller}), cuenta con una variedad de argumentos que pueden ser modificados para la ejecución de la simulación.

\begin{itemize}
	\item \textbf{tSim}: Tiempo de Simulación. tSim=10 por omisión.
	\item \textbf{InArrTime}: Mean Interarrival Time. InArrTime=4.0 por omisión.
	\item \textbf{MeSerTime}: Mean Service Time. MeSerTime=3.50 por omisión.
	\item \textbf{DeSerTime}: Deviation of Mean Service Time. DeSerTime=0.80 por omisión.
	\item \textbf{stim}: "tiempo máximo de simulación".
	\item \textbf{seedN}: "semilla del generador de números aleatorios N".
\end{itemize}

Para simplicidad de las pruebas fueron escogidos tres valores a ser modificados: 1)\textbf{tSim}: el tiempo de simulación variando desde su valor por omisión hasta diez veces ese valor (100), 2)\textbf{InArrTime}: tiempo entre llegadas variando desde su valor por omisión hasta el doble de ese valor(8.0) y 3)\textbf{MeSerTime}: tiempo de servicio variando desde su valor por omisión hasta el doble de ese valor(7.0).

Cada variación de los argumentos se realizó por separado, mientras un argumento era modificado el resto de los mismos trabajaban con su valor por omisión.

\subsection{Resultados}

\subsubsection{tSim}

Comenzando el tSim=10 y terminando con un tSim=100, con un incremento de 10. Se realizaron 10 pruebas, obteniendo en todas la misma información entre simulaciones. En la figura \ref{pruebatSimA} se observa la ejecución del modelo usando GALATEA en la consola y en la figura \ref{pruebatSimB} se observa el resultado de ejecutar el mismo modelo pero mediante el sistema web de GALATEA.

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.4]{img/tSim10.png}
	\caption{tSim=10 Consola.}
	\label{pruebatSimA}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.4]{img/tSim10S.png}
	\caption{tSim=10 Sistema web.}
	\label{pruebatSimB}
\end{figure}


\subsubsection{InArrTime}

Comenzando el InArrTime=4.0 y terminando con un InArrTime=8.0, con un incremento de 1.0. Se realizaron 4 pruebas, obteniendo en todas la misma información entre simulaciones. En la figura \ref{pruebatSimC} se observa la ejecución del modelo usando GALATEA en la consola y en la figura \ref{pruebatSimD} se observa el resultado de ejecutar el mismo modelo pero mediante el sistema web de GALATEA.

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.4]{img/InArrTime8.png}
	\caption{InArrTime=8.0 Consola.}
	\label{pruebatSimC}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.4]{img/InArrTime8S.png}
	\caption{InArrTime=8.0 Sistema web.}
	\label{pruebatSimD}
\end{figure}

\subsubsection{MeSerTime}

Comenzando el MeSerTime=3.5 y terminando con un MeSerTimerTime=7.5, con un incremento de 1.0. Se realizaron 4 pruebas, obteniendo en todas la misma información entre simulaciones. En la figura \ref{pruebatSimE} se observa la ejecución del modelo usando GALATEA en la consola y en la figura \ref{pruebatSimF} se observa el resultado de ejecutar el mismo modelo pero mediante el sistema web de GALATEA.

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.4]{img/MeSerTime7.png}
	\caption{MeSerTime=7.5 Consola.}
	\label{pruebatSimE}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.4]{img/MeSerTime7S.png}
	\caption{MeSerTime=7.5 Sistema web.}
	\label{pruebatSimF}
\end{figure}

Observamos que todas las simulaciones son análogas, obtenemos los mismos resultados en todas las simulaciones que realizamos. Por lo que concluimos que nuestra integración se realizó exitosamente.

\section{Pruebas de Integración con GALATEA (Sockets)}

La integración con GALATEA mediante sockets toma como base el modelo \textbf{SimpleTeller} para su funcionamiento. El objetivo principal de la integración es verificar la interacción entre el proceso de simulación y el cliente en tiempo real. Mediante el protocolo de comunicación desarrollado, enviar instrucciones a la simulación y que sean ejecutadas sin problemas, al igual obtener las respuestas enviadas por el proceso de simulación.

Para verificar el correcto funcionamiento de esta integración usando sockets, se definieron tres clientes o casos de pruebas distintos: Un cliente en lenguaje ``Python", un cliente en lenguaje ``C" y un cliente integrado en el sistema web de simulación.

\subsection{Cliente Python}

El cliente Python se desarrolló, para ejecutar cuatro instrucciones básicas del protocolo de comunicación: ``Start" comenzar la simulación, ``Set" modificar el valor de una variable dentro de la simulación, ``Get" obtener el valor de una variable dentro de la simulación y ``Exit" terminar la simulación. En la figura \ref{pythonClient} observamos la ejecución de nuestro cliente en Python, mientras en la figura \ref{integracionGalateaRespuestaA} observamos la respuesta de la simulación en GALATEA usando ese cliente Python.

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.5]{img/TestIntegrationPythonA.png}
	\caption{Cliente Python.}
	\label{pythonClient}
\end{figure}

En nuestro cliente Python observamos la ejecución de algunas de las instrucciones definidas en nuestro protocolo de comunicación como lo son: Start, Set, Get y Exit para detener la simulación.

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.4]{img/TestIntegrationPythonB.png}
	\caption{Respuesta Simulación GALATEA.}
	\label{integracionGalateaRespuestaA}
\end{figure}

\subsection{Cliente C}

El cliente C se desarrolló, para ejecutar cuatro instrucciones básicas del protocolo de comunicación al igual que el cliente Python: ``Start" comenzar la simulación, ``Set" modificar el valor de una variable dentro de la simulación, ``Get" obtener el valor de una variable dentro de la simulación y ``Exit" terminar la simulación. En la figura \ref{cClient} observamos la ejecución de nuestro cliente en C, mientras en la figura \ref{integracionGalateaRespuestaB} observamos la respuesta de la simulación en GALATEA usando ese cliente en C.

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.5]{img/TestIntegrationCA.png}
	\caption{Cliente en C.}
	\label{cClient}
\end{figure}

En nuestro cliente C observamos la ejecución de algunas de las instrucciones definidas en nuestro protocolo de comunicación como lo son: Start, Set, Get y Exit para detener la simulación.

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.4]{img/TestIntegrationCB.png}
	\caption{Respuesta Simulación GALATEA.}
	\label{integracionGalateaRespuestaB}
\end{figure}

Observamos que ambos clientes se comunican con el proceso de simulación. Y el protocolo de comunicación funciona como se espera al igual el proceso de simulación envía las respuestas esperadas y ejecuta la simulación del modelo utilizado.

\subsection{Cliente Web}

El cliente web, es una interfaz web que permite le ejecución de la simulación y el envío de las instrucciones de comunicación para en control de la simulación. Estas instrucciones se realizan mediante peticiones AJAX a métodos Python, los cuales a su vez se conectan al socket específico de la simulación y envían las instrucciones dadas. En la figura \ref{webClient} observamos la ejecución de nuestro cliente Web, mientras en la figura \ref{integracionGalateaRespuestaC} observamos la respuesta de la simulación en GALATEA usando ese cliente Web.

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.3]{img/TestIntegrationWebA.png}
	\caption{Cliente Web.}
	\label{webClient}
\end{figure}
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.4]{img/TestIntegrationWebB.png}
	\caption{Respuesta Simulación GALATEA.}
	\label{integracionGalateaRespuestaC}
\end{figure}

El protocolo de comunicación funciona como se espera al igual el proceso de simulación envía las respuestas esperadas y ejecuta la simulación del modelo utilizado.


\section{Pruebas manejo de Archivo y Carpetas}

\subsection{Manejo de Archivos}

La creación de archivos es un objetivo fundamental dentro del proyecto, por lo que es necesario garantizar su correcto funcionamiento. La prueba consistió en la creación de archivos usando el sistema web y verificar su correcta creación en el servidor. En la figura \ref{pruebatCreateA} observamos la creación del archivo mediante nuestro sistema web.

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.4]{img/PruebaCrearArchivo.png}
	\caption{Crear un archivo de prueba mediante sistema web.}
	\label{pruebatCreateA}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.4]{img/PruebaCrearArchivoS.png}
	\caption{Archivo ModeloPrueba.g verificación de existencia.}
	\label{pruebaCreateAS}
\end{figure}

Observamos que el archivo \textbf{ModeloPrueba.g} fue creado con éxito. Y se encuentra dentro del directorio principal del admin como se muestra en la figura \ref{pruebaCreateAS}.

\subsection{Manejo de Carpetas}

La creación de carpetas para el control de proyectos y modelos es un objetivo fundamental dentro del proyecto, por lo que es necesario garantizar su correcta creación y localización dentro del sistema. La prueba consistió en la creación de caretas usando el sistema web y verificar su correcta creación y localización en el servidor. En la figura \ref{pruebatCreateF} observamos la creación de la carpeta mediante nuestro sistema web.

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.4]{img/PruebaCrearCarpeta.png}
	\caption{Crear una carpeta de prueba mediante sistema web.}
	\label{pruebatCreateF}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.4]{img/PruebaCrearCarpetaS.png}
	\caption{Carpeta FolderTest verificación de existencia.}
	\label{pruebaCreateFS}
\end{figure}

Observamos que la carpeta \textbf{FolderTest} fue creada con éxito. Y se encuentra dentro del directorio principal del admin como se muestra en la figura \ref{pruebaCreateFS}.

\section{Pruebas del Sistema WEB}
...
\begin{enumerate}
	\item \textbf{Archivos estáticos con Apache}: En esta prueba se configuró un servidor Apache para servir sólo páginas web y archivos estáticos. Se encontró un valor de 640 peticiones por minuto como valor óptimo.
	\item \textbf{Páginas dinámicas con Django sobre Apache}: En esta prueba se configuro Apache para ejecutar sólo la aplicación web Django a través del módulo \textit{mod\_wsgi}, obteniendo un promedio de 768 peticiones por minuto como valor óptimo.	
	\item \textbf{Páginas dinámicas con Django y archivos estáticos sobre el mismo servidor Apache}: Se observó un valor óptimo de 128 peticiones por minuto.
\end{enumerate}
...
\begin{table}[H]
	...
	\begin{tabular}{p{2.5cm} p{10cm}}
		\hline
		\textbf{U-07} & \textbf{Control de Simulación asíncrono} \\
		\hline \hline
...
\begin{table}[H]
	...
	\begin{tabular}{p{2.5cm} p{10cm}}
		\hline
		\textbf{U-07} & \textbf{Control de Simulación Síncrono} \\
		\hline \hline
		\textbf{Precondición} & Usuario autentificado. Archivo de simulación válido. \\
		\hline
		\textbf{Descripción} & Se compila y ejecuta un archivo de simulación. La información resultante se observa en el sistema. \\
		\hline
		\textbf{Secuencia Normal} & Se visualiza un archivo de simulación válido. Click en ejecutar. Archivo compilado y ejecutado. Se muestra las opciones de ejecución: Start, Stop, Pause, Yield, Set y Get. Se procede a controlar la simulación enviándole esas instrucciones y obteniendo los resultados en tiempo real. \\
		\hline
		\textbf{Excepciones} & Archivo no válido. Error en compilación. Error en Archivo. \\
		\hline
		\textbf{Postcondición} & Resultado de la compilación y ejecución por pantalla. \\
		\hline
	\end{tabular}
	\caption{Caso de uso - Control de Simulación Síncrono.}
	\label{tabla:casouso021}
\end{table}
...
\chapter{Instalación y Configuración del Sistema web}

\section{Prerequisites}

The first thing is to get a Debian 8 Stable system going. This tutorial assumes you have a Debian 8 Stable Droplet ready. Except otherwise noted, all of the commands in this tutorial should be run as a non-root user with sudo privileges.

\section{Install packages from Debian repositories}

To begin the process, we'll download and install all of the items we need from the Debian repositories. This will include the Apache web server, the mod\_wsgi module used to interface with our Django app, and pip, the Python package manager that can be used to download our Python-related tools.
...
Before installing PostgreSQL, make sure that you have the latest information from the Debian repositories by updating the apt package
\begin{lstlisting}
...
\end{lstlisting}
\subsection{Install Django-Roles-Permission for control of roles on the system}
\begin{lstlisting}
sudo pip install django-role-permissions
\end{lstlisting}
\subsection{Install Drivers for MySQL}
\begin{lstlisting}
...
\end{lstlisting}
\subsection{Install drivers for pillow, image manager}
\begin{lstlisting}
sudo pip install pillow
\end{lstlisting}
\subsection{Install html2text for html manage}
\begin{lstlisting}
sudo pip install html2text
\end{lstlisting}
\subsection{Install Pusher for realtime notifications}
\begin{lstlisting}
sudo pip install pusher
\end{lstlisting}
\subsection{Install and Configure java 8}
Use this link http://www.webupd8.org/2014/03/how-to-install-oracle-java-8-in-debian.html

\subsection{Install and Configure GALATEA}
Use this link http://galatea.sourceforge.net/Home.htm

\section{Configure Project}
Clone de project from Github: I use \textbf{/var/www/html/} like root folder.
\begin{lstlisting}
git clone https://github.com/velasquezerik/django-roles-test.git
\end{lstlisting}
...
SHOW DATABASES;
\end{lstlisting}
\subsection{Configure Project}
Copy secrets\_example to secrets and configure:
\begin{lstlisting}
cd /path/to/project/
cd django-roles-test/tesis/
...
E_EMAIL_PORT = 587
E_EMAIL_HOST_USER = "email@email.com"
E_EMAIL_HOST_PASSWORD = "password"
E_EMAIL_USE_TLS = True
\end{lstlisting}
\subsection{Configure Media Folder}
Open the file \textbf{settings.py} and configure this lines:
\begin{lstlisting}
MEDIA_ROOT = os.path.join(BASE_DIR, 'media')
MEDIA_URL = '/media/'
\end{lstlisting}
Actual Path is: \textbf{/path/to/project/django-roles-test/media/}

Note: Leave that folder address. If you change the address, copy the \textbf{media} folder to that new path. 
\subsection{Run Migrations}
\begin{lstlisting}
cd /path/to/project/django-roles-test/
python manage.py migrate
\end{lstlisting}
\subsection{Create Superuser}
\begin{lstlisting}
cd /path/to/project/django-roles-test/
python manage.py createsuperuser
\end{lstlisting}
\subsection{Create Image for Superuser}
\begin{lstlisting}
mysql -p username
SHOW DATABASES;
USE project-database;
INSERT INTO website_userimage ('model_pic', 'user_id') VALUES ('profiles/profile.png', '1')
\end{lstlisting}
\subsection{Create Folder for Superuser}
\begin{lstlisting}
mysql -p username
SHOW DATABASES;
USE project-database;
INSERT INTO website_folder ( 'name', 'path', 'father', 'active', 'user_id', 'permission') VALUES ('superuser-username', '/path/to/media/disk/superuser-username', '0', '1', '1', '0');
\end{lstlisting}
\subsection{Create DiskSpace for Superuser}
\begin{lstlisting}
mysql -p username
SHOW DATABASES;
USE project-database;
INSERT INTO website_diskspace ('user_id', 'max_space', 'usage') VALUES ('1', '1024', '0')
\end{lstlisting}

\section{Configure Apache project}
Now that your Django project is working, we can configure Apache as a front end. Client connections that it receives will be translated into the WSGI format that the Django application expects using the mod\_wsgi module. This should have been automatically enabled upon installation earlier.
...
<VirtualHost *:80>
	...
	Alias /static /path/to/project/django-roles-test/website/static
	<Directory /path/to/project/django-roles-test/website/static>
		Require all granted
	</Directory>
	
	<Directory /path/to/project/django-roles-test/tesis>
		<Files wsgi.py>
			Require all granted
		</Files>
	</Directory>
	
	WSGIDaemonProcess tesis
	WSGIProcessGroup tesis
	WSGIScriptAlias / /path/to/project/django-roles-test/tesis/wsgi.py
</VirtualHost>
\end{lstlisting}
Restart Apache
\begin{lstlisting}
...
\section{Links}
\begin{itemize}
	\item https://docs.djangoproject.com/es/1.10/howto/deployment/wsgi/modwsgi/
	\item https://modwsgi.readthedocs.io/en/develop/user-guides/quick-configuration-guide.html
	\item https://www.digitalocean.com/community/tutorials/how-to-install-and-use-postgresql-9-4-on-debian-8
	\item https://www.digitalocean.com/community/tutorials/how-to-serve-django-applications-with-apache-and-mod\_wsgi-on-ubuntu-14-04
	\item https://www.digitalocean.com/community/tutorials/how-to-install-git-on-debian-8
	\item https://www.digitalocean.com/community/tutorials/how-to-install-and-configure-django-with-postgres-nginx-and-gunicorn
	\item http://www.pdxpixel.com/blog/2014/02/04/setting-up-django-site-ssl-apache-mod\_wsgi-mod\_ssl/
	\item https://github.com/teddziuba/django-sslserver
	\item https://docs.djangoproject.com/en/1.10/topics/security/
	\item http://stackoverflow.com/questions/21728951/django-and-ssl-server
\end{itemize}
...
\chapter{Manual de usuario del Sistema web}

El sistema web de simulación (en la figura \ref{paginaInicio} observamos la pantalla de inicio del sistema web), cuenta con una gama de funcionalidades que explicaremos a continuación:

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.4]{img/Tesis/1TesisInicio.png}
	\caption{Página de inicio del Sistema Web.}
	\label{paginaInicio}
\end{figure}

Para ingresar al sistema debemos colocar nuestras credenciales (Username y password), como se observa en la figura \ref{paginaInicioLogin}.

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.4]{img/Tesis/2TesisLogin.png}
	\caption{Página de Login al Sistema Web.}
	\label{paginaInicioLogin}
\end{figure}


\section{Manejo de Usuarios}

El sistema permite el control de dos tipos de usuarios (Administradores y usuarios normales. En la figura \ref{paginaInicioAdmin} se observa la pantalla de inicio de una usuario administrador y en la figura \ref{paginaInicioUser} se observa la pantalla de inicio de un usuario normal.

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.2]{img/Tesis/3TesisAdminIndex.png}
	\caption{Página de Inicio Usuario Administrador.}
	\label{paginaInicioAdmin}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.2]{img/Tesis/4TesisUserIndex.png}
	\caption{Página de Inicio Usuario Normal.}
	\label{paginaInicioUser}
\end{figure}
\subsection{Control de Usuarios}

Solo los administradores del sistema pueden acceder a esta página y visualizar los usuarios. Contamos con una serie de opciones en esta página. El botón "\textbf{New Admin}" y "\textbf{New User}" como se observa en la figura \ref{paginaListaUsuarios}, te envían a la pagina respectiva para crear ese tipo de usuario. Cada uno de los usuarios listados, cuentan con la opción de "\textbf{Delete}" y "\textbf{Edit}", que desactiva al usuario del sistema y permite editar sus datos respectivamente.

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.2]{img/Tesis/5TesisAdminUsers.png}
	\caption{Lista de usuarios del Sistema Web.}
	\label{paginaListaUsuarios}
\end{figure}

\subsubsection{Agregar Usuario normal}

Agregar un usuario con los privilegios de usuario normal del sistema (como se observa en la figura \ref{paginaAgregarUser}). Ésta página la accedemos desde el botón "\textbf{New User}" en la página \ref{paginaListaUsuarios}.

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.2]{img/Tesis/6TesisAdminNewUser.png}
	\caption{Agregar Usuario.}
	\label{paginaAgregarUser}
\end{figure}

\subsubsection{Agregar Usuario Administrador}

Agregar un usuario con los privilegios de administrador del sistema (como se observa en la figura \ref{paginaAgregarAdmin}).  Ésta página la accedemos desde el botón "\textbf{New Admin}" en la página \ref{paginaListaUsuarios}.

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.2]{img/Tesis/7TesisAdminNewAdmin.png}
	\caption{Agregar Administrador.}
	\label{paginaAgregarAdmin}
\end{figure}

\subsubsection{Editar Usuario}

Editamos la información principal del usuario del sistema (como se observa en la figura \ref{paginaEditUser}), al igual la cantidad de espacio en disco asociado. Ésta página la accedemos desde el botón "\textbf{Editar}" en la página \ref{paginaListaUsuarios}, asociado a cada usuario.

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.2]{img/Tesis/8TesisAdminEditUsers.png}
	\caption{Editar Información de un usuario.}
	\label{paginaEditUser}
\end{figure}

\subsection{Logs del Sistema}

Observamos los logs de archivos (como se observa en la figura \ref{paginaLogsFiles}). Los cambios principales de archivos que los usuarios han realizado en el sistema. Ésta página la accedemos desde el botón "\textbf{Logs Files}" en la página \ref{paginaInicioAdmin}. Solo Usuarios con el rol de \textbf{Administradores} pueden acceder a ella.

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.2]{img/Tesis/9TesisAdminLogsFiles.png}
	\caption{Logs de Archivos.}
	\label{paginaLogsFiles}
\end{figure}

Observamos los logs de carpetas (como se observa en la figura \ref{paginaLogsFolders}). Los cambios principales de carpetas que los usuarios han realizado en el sistema. Ésta página la accedemos desde el botón "\textbf{Logs Folders}" en la página \ref{paginaInicioAdmin}. Solo Usuarios con el rol de \textbf{Administradores} pueden acceder a ella.

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.2]{img/Tesis/10TesisAdminLogsFolders.png}
	\caption{Logs de Carpetas.}
	\label{paginaLogsFolders}
\end{figure}

Observamos los logs de relaciones (como se observa en la figura \ref{paginaLogsRelations}). Los cambios principales de las relaciones que los usuarios han realizado en el sistema. Ésta página la accedemos desde el botón "\textbf{Logs Relations}" en la página \ref{paginaInicioAdmin}. Solo Usuarios con el rol de \textbf{Administradores} pueden acceder a ella.

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.2]{img/Tesis/11TesisAdminLogsRelations.png}
	\caption{Logs de Relaciones.}
	\label{paginaLogsRelations}
\end{figure}

\subsection{Perfil de Usuario}

Modificamos nuestra información de perfil, datos básicos y contraseña. Accedemos dando click en la \textbf{esquina superior derecha}, como se observa en la figura \ref{paginaUserGetProfile} en la opción "\textbf{Profile}".

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.4]{img/Tesis/29TesisUserGetProfile.png}
	\caption{Acceder al Perfil de Usuario.}
	\label{paginaUserGetProfile}
\end{figure}

En el perfil de usuario nos encontramos con la información básica del usuario. Podemos modificar los \textbf{datos básicos} o \textbf{cambiar la contraseña} como se observa en la figura \ref{paginaUserProfile}.

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.2]{img/Tesis/12TesisUserProfile.png}
	\caption{Perfil de Usuario.}
	\label{paginaUserProfile}
\end{figure}

\subsection{Manejo de Amigos}

Listado de nuestros amigos en el sistema (como se observa en la figura \ref{paginaUserFriends}). Ésta página la accedemos desde el botón "\textbf{Friends}" en la página \ref{paginaInicioUser}.

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.2]{img/Tesis/13TesisUserFriends.png}
	\caption{Listado de Amigos.}
	\label{paginaUserFriends}
\end{figure}

\subsubsection{Obtener nuevos amigos}

Enviamos notificaciones de amistad a usuarios (como se observa en la figura \ref{paginaUserGetFriends}). Ésta página la accedemos desde el botón "\textbf{ Get Friends}" en la página \ref{paginaInicioUser}.

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.2]{img/Tesis/14TesisUserGetFriends.png}
	\caption{Agregar nuevos Amigos.}
	\label{paginaUserGetFriends}
\end{figure}

\subsubsection{Notificaciones amigos}

Verificamos las notificaciones de amistad que nos han enviado (como se observa en la figura \ref{paginaUserFriendNotification}). Ésta página la accedemos desde el botón "\textbf{Friends Notifications}" en la página \ref{paginaInicioUser}.

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.2]{img/Tesis/15TesisUserFriendNotification.png}
	\caption{Notificaciones de Amigos.}
	\label{paginaUserFriendNotification}
\end{figure}

\section{Manejo de Archivos y Carpetas}

\subsection{Archivos Compartidos}

Los usuarios en el sistema pueden compartir archivos entre ellos.

\subsubsection{Notificaciones de Archivos Compartidos}

Verificamos las notificaciones de archivos compartidos que nos han enviado (como se observa en la figura \ref{paginaUserShareNotification}). Ésta página la accedemos desde el botón "\textbf{Share Files Notifications}" en la página \ref{paginaInicioUser}.

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.2]{img/Tesis/16TesisUserShareNotifications.png}
	\caption{Notificaciones de Archivos Compartidos.}
	\label{paginaUserShareNotification}
\end{figure}

\subsubsection{Acceso a los Archivos Compartidos}

Accedemos a nuestros archivos compartidos (como se observa en la figura \ref{paginaUserShareFiles}). Ésta página la accedemos desde el botón "\textbf{Share Files}" en la página \ref{paginaInicioUser}.

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.2]{img/Tesis/17TesisUserShareFiles.png}
	\caption{Archivos Compartidos con el usuario.}
	\label{paginaUserShareFiles}
\end{figure}

\subsection{Crear Carpetas}

Creamos nuevas carpetas. Ésta página la accedemos desde el botón "\textbf{New Folder}" en la página \ref{paginaInicioUser}. Colocamos el nombre de la nueva carpeta y click la opción "\textbf{Create}" como se observa en la figura \ref{paginaUserCreateFolder}.

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.4]{img/Tesis/18TesisUserCreateFolder.png}
	\caption{Creación de Carpetas.}
	\label{paginaUserCreateFolder}
\end{figure}

\subsection{Crear Archivos}

Creamos nuevos Archivos. Ésta página la accedemos desde el botón "\textbf{New File}" en la página \ref{paginaInicioUser}. Colocamos el nombre del nuevo Archivo y click la opción "\textbf{Create}" como se observa en la figura \ref{paginaUserCreateFile}.

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.4]{img/Tesis/19TesisUserCreateFile.png}
	\caption{Creación de Archivos.}
	\label{paginaUserCreateFile}
\end{figure}

\subsection{Cargar Archivos}

Cargamos nuevos Archivos. Ésta página la accedemos desde el botón "\textbf{Upload File}" en la página \ref{paginaInicioUser}. Buscamos el nuevo Archivo y click la opción "\textbf{Upload}" como se observa en la figura \ref{paginaUserUploadFile}.

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.4]{img/Tesis/20TesisUserUploadFile.png}
	\caption{Cargar Archivos.}
	\label{paginaUserUploadFile}
\end{figure}

\subsection{Crear Proyecto de Simulación}

Creamos un nuevo proyecto de simulación. Ésta página la accedemos desde el botón "\textbf{Create Project}" en la página \ref{paginaInicioUser}. Colocamos el nombre del nuevo proyecto y click la opción "\textbf{Create}" como se observa en la figura \ref{paginaUserCreateProject}. Esto creará una nueva carpeta con el \textbf{nombre del proyecto} y un nuevo \textbf{modelo de simulación} dentro de esa carpeta. El nuevo modelo cuenta con la estructura e información \textbf{básica de una taquilla}.

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.4]{img/Tesis/21TesisUserCreateProject.png}
	\caption{Crear Proyecto de Simulación.}
	\label{paginaUserCreateProject}
\end{figure}

\subsection{Mostrar Archivo}

Accedemos a la información que contienen los archivos dentro del sistema (como se observa en la figura \ref{paginaUserShowFile}). Nos permite \textbf{editarla} y \textbf{guardarla}. Accedemos dándole click al botón a un archivo en la página \ref{paginaInicioUser}.

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.2]{img/Tesis/22TesisUserShowFile.png}
	\caption{Mostrar Archivo.}
	\label{paginaUserShowFile}
\end{figure}

\subsection{Permisos de Archivo}

Modificamos la permisología de acceso del archivo (como se observa en la figura \ref{paginaUserFilePermissions}). Accedemos dándole click al botón "\textbf{File Permission}" en la página \ref{paginaUserShowFile}. Nos permite manejar cuatro permisologías (\textbf{Private, show, edit y Public}).

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.2]{img/Tesis/28TesisUserFilePermissions.png}
	\caption{Permisos de Archivo.}
	\label{paginaUserFilePermissions}
\end{figure}



\section{Ejecución de GALATEA}

\subsection{Mostrar opciones de Compilación y Ejecución}

Observamos las opciones de compilación que posee el archivo (como se observa en la figura \ref{paginaUserCompileExecute}). Accedemos a estas opciones en la página \ref{paginaUserShowFile}. 

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.2]{img/Tesis/23TesisUserShowCompileExecuteFile.png}
	\caption{Opciones de Compilación y Ejecución.}
	\label{paginaUserCompileExecute}
\end{figure}


\subsection{Ejecutar Modelo de Simulación}

Ejecutamos el modelo de simulación dándole click a la opción "\textbf{Execute File}" y pasando los argumentos que se deseen al modelo en el input "\textbf{Arguments}" (como se observa en la figura \ref{paginaUserExecuteFile}). Accedemos a estas opciones en la página \ref{paginaUserShowFile}. 

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.2]{img/Tesis/24TesisUserExecuteFile.png}
	\caption{Ejecutar Modelo de Simulación.}
	\label{paginaUserExecuteFile}
\end{figure}

El botón "\textbf{Export Data}" nos permite crear un nuevo archivo con la información de salida de nuestra simulación.
